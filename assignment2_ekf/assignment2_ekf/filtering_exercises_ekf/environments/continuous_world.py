...
class ContinuousWorld:
    ...
    def __init__(
        ...
        # added by Scott
        self.station = np.array([8.0, 2.0])
        self.station_2 =  np.array([5.0, 6.0])  # ranging station (change as you like)
        self.station_noise_std = 0.05         # meters, measurement noise
        # self.landmarks = [self.station] 
        self.landmarks = [self.station, self.station_2]       # UNCOMMENT FOR 2 STATION IMPLEMENTATION
    ...
    def range_to_station_2(self, pos: np.ndarray):
        
        dx, dy = pos[0] - self.station_2[0], pos[1] - self.station_2[1]
        return float(np.hypot(dx, dy))
    
    def noisy_range_to_station_2(self, pos: np.ndarray):
        r = self.range_to_station_2(pos)
        return float(r + np.random.normal(0.0, self.station_noise_std))

    def noisy_range_to_station(self, pos: np.ndarray) -> float:
        """Noisy range measurement to the FIRST station (for tests)."""
        r = self.range_to_station(pos)
        return float(r + np.random.normal(0.0, self.station_noise_std))

    def noisy_range_to_2_stations(self, pos): # UNCOMMENT FOR 2 STATION IMPLEMENTATION
        r1 = float(self.range_to_station(pos) + np.random.normal(0, self.station_noise_std))
        r2 = float(self.range_to_station_2(pos) + np.random.normal(0, self.station_noise_std))
