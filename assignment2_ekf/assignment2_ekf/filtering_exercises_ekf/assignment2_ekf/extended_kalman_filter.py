...
class ExtendedKalmanFilter:
    ...
    def _motion_model(self, state: np.ndarray, forward_vel: float, angular_vel: float, dt: float) -> np.ndarray:
        ...
        heading_next = (heading + effective_angular_vel * dt) % (2 * np.pi)
    ...
    def _measurement_jacobian(self, state: np.ndarray) -> np.ndarray:
        ...
        dx = state[0] - self.env.station[0]
        dy = state[1] - self.env.station[1]
        r = np.sqrt(dx ** 2 + dy ** 2)
        dr_dtheta = 0.0
        dr_dx = (dx/r)
        dr_dy = (dy/r)
    ...
    def update(self, measurements: np.ndarray):
        ...
        expected_z = self._measurement_model(self.state).reshape(1,)
        H = self._measurement_jacobian(self.state)
        innovation = H @ self.covariance @ H.T + self.R
        S_inv = np.linalg.inv(innovation)
        K = self.covariance @ H.T @ S_inv
        
        y = (z - expected_z).reshape(1, 1)
        dx = (K @ y).reshape(-1)
        dx[0] = np.clip(dx[0], -self.max_position_update, self.max_position_update)
        dx[1] = np.clip(dx[1], -self.max_position_update, self.max_position_update)
        self.state = self.state + dx
        self._normalize_heading()
        I = np.eye(self.n_states)
        KH = K @ H                                      
        self.covariance = (I - KH) @ self.covariance @ (I - KH).T + K @ self.R @ K.T
...
class ExtendedKalmanFilter:
    ...
    def _measurement_model(self, state: np.ndarray) -> np.ndarray:
        # z = range to the known station 
        r1 = self.env.range_to_station(state[:2])
        r2 = self.env.range_to_station_2(state[:2])
    ...
    def _measurement_jacobian(self, state: np.ndarray) -> np.ndarray:
        ...
        """
        # raise NotImplementedError("TODO: Implement the measurement Jacobian (H = ∂h/∂x).")\
        dx1 = state[0] - self.env.station[0]
        dy1 = state[1] - self.env.station[1]
        r1 = np.sqrt(dx1 ** 2 + dy1 ** 2)

        dx2 = state[0] - self.env.station_2[0]
        dy2 = state[1] - self.env.station_2[1]
    ...
    def update(self, measurements: np.ndarray):
        ...
        """
        # raise NotImplementedError("TODO: Implement the Extended Kalman Filter update step")
        z = np.asarray(measurements)

        if z.size == 1:
            z = z.reshape(1,)
            expected_all = self._measurement_model(self.state)          # [r1, r2]
            expected_z = np.array([expected_all[0]])
            H_all = self._measurement_jacobian(self.state)              # 2×3
            H = H_all[0:1, :]                                           # 1×3
            R = self.R                                                  # 1×1
        elif z.size == 2:
            z = z.reshape(2,)
            expected_z = self._measurement_model(self.state).reshape(2,)
            H = self._measurement_jacobian(self.state)                  # 2×3
            R = np.diag([self.R[0, 0], self.R[0, 0]])                   # 2×2

        innovation = H @ self.covariance @ H.T + R
        S_inv = np.linalg.inv(innovation)
        K = self.covariance @ H.T @ S_inv
        ...
        I = np.eye(self.n_states)
        KH = K @ H
        self.covariance = (I - KH) @ self.covariance @ (I - KH).T + K @ R @ K.T
