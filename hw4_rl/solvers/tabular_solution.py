...
class GridworldSolver:
    ...
    def __init__(
        ...
        self.gamma = 0.9 #future return discount factor
        self.eps = 1e-12
            ...
            for s in range(unwrapped_env.num_states):
                ...
                for a in unwrapped_env.actions:
                    ...
                    for s_next in range(unwrapped_env.num_states):
                        action_value_sa += (
                            T[s, a, s_next] * R[s, a, s_next]
                            + T[s, a, s_next] * self.gamma * v_temp[s_next]
                        )
                    action_values.append(action_value_sa)
                v_i[s] = max(action_values)

            delta = np.max(np.abs(v_i - v_temp))
            ...
            for s in range(unwrapped_env.num_states):
                ...
                for a in unwrapped_env.actions:
                    ...
                    for s_next in range(unwrapped_env.num_states):
                        q_sa_values += (
                            T[s, a, s_next] * R[s, a, s_next]
                            + T[s, a, s_next] * self.gamma * v_i[s_next]
                        )
    ...
    def _deterministic_policy_iteration(self) -> None:
        ...
        use_exact_eval = False # Bonus Question
        ...
        for k in range(50):
            ...
            elapsed = time.time()
            if use_exact_eval:
                S = unwrapped_env.num_states
                policy_prob_exact = (p_i[:, :, None] * T).sum(axis=1)           
                reward_exact = (p_i[:, :, None] * T * R).sum(axis=(1, 2))  
                v_i = np.linalg.solve(np.eye(S) - self.gamma * policy_prob_exact, reward_exact)
            # Policy Evaluation
            else:
                v_i = np.zeros(unwrapped_env.num_states)
            ...
            self.solver.set_value_function(v_i)
            self.solver.set_policy_function(p_new)
            self.performance_history.append(self.solve(max_steps=20)[0])        
...
if __name__ == "__main__":
    ...
    for solver in [gw0_det_solver, gw1_det_solver]:
        print("Starting!")
        start_time = time.time()
        solver.compute_policy()  # run VI here
        elapsed_time = time.time() - start_time
