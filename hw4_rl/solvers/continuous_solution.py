...
student_name = "Anudeep Vedantam"  # Set to your name
...
class TabularPolicy:
    ...
    def get_action(self, state: int) -> int:
        ...
        prob = np.asarray(self._policy[state], dtype=float)
        prob = np.clip(prob, 0.0, None)           # no negatives
        s = prob.sum()
        if not np.isfinite(s) or s <= 0.0:
    # fallback to uniform if this row is all zeros/NaN
...
class DiscretizedSolver:
    ...
    def __init__(
        ...
        assert policy_type in ["deterministic_vi", "deterministic_pi"]
        ...
        S = self.solver.num_states
        A = self.solver.num_actions
        uniform_pi = np.ones((S, A), dtype=float) / A
        self.solver.set_policy_function(uniform_pi)
    ...
    def get_discrete_state_probabilities(self, continuous_state: np.ndarray) -> List[Tuple[int, float]]:
        ...
        """
        if self._mode == "nn":
            pos_vel_state = np.array(continuous_state, dtype=float)
            pos_vel_state = np.clip(pos_vel_state, self.state_lower_bound, self.state_upper_bound - 1e-10)
            grid_center_value = (pos_vel_state - self.state_lower_bound) / self.bin_sizes - 0.5

            nearest_grid_centers = np.round(grid_center_value).astype(int)
            nearest_grid_centers = np.clip(nearest_grid_centers, 0, self._num_bins - 1)
            pos_index = nearest_grid_centers[0]
            vel_index = nearest_grid_centers[1]
            state_index = pos_index * self._num_bins + vel_index
            ...
            pos_vel_state = np.clip(pos_vel_state, self.state_lower_bound, self.state_upper_bound - 1e-10)
            grid_center_value = (pos_vel_state - self.state_lower_bound) / self.bin_sizes - 0.5
            rounded = np.round(grid_center_value)
            '''if grid_center_value[0] < np.round(grid_center_value[0]):
                lower_corner[0] = np.round(grid_center_value[0]) - 1
            elif grid_center_value[0] > np.round(grid_center_value[0]):
                lower_corner[0] = np.round(grid_center_value[0])
            if grid_center_value[1] < np.round(grid_center_value[1]):
                lower_corner[1] = np.round(grid_center_value[1]) - 1
            elif grid_center_value[1] > np.round(grid_center_value[1]):
                lower_corner[1] = np.round(grid_center_value[1])'''
            
            lower_corner = np.zeros_like(grid_center_value, dtype=int)
            higher_corner = np.zeros_like(grid_center_value, dtype=int)
            alpha = np.zeros_like(grid_center_value)

            for i in range(self.num_dims):
                lower_corner[i] = int(np.floor(grid_center_value[i]))
                higher_corner[i] = lower_corner[i] + 1
                alpha[i] = grid_center_value[i] - lower_corner[i]

            


            for dimension in range(self.num_dims):
                if lower_corner[dimension] < 0:
                    lower_corner[dimension] = 0
                if higher_corner[dimension] >= self._num_bins:
                    higher_corner[dimension] = self._num_bins - 1
                if lower_corner[dimension] == 0 or higher_corner[dimension] == self._num_bins - 1:
                    lower_corner[dimension] = higher_corner[dimension]
                    alpha[dimension] = 0.0
            ...
            neighbours = []
            for i in [int(lower_corner[0]), int(higher_corner[0])]:
                for j in [int(lower_corner[1]), int(higher_corner[1])]:
                    neighbours.append((i, j))
            valid_neighbours = []
            for corner in neighbours:
                center = self.state_lower_bound + (np.array(corner) + 0.5) * self.bin_sizes
                within_bounds_bool = True
                for dimension in range(self.num_dims):
                    if center[dimension] < self.state_lower_bound[dimension] - 1e-10:
                        within_bounds_bool = False
                        break
                    if center[dimension] > self.state_upper_bound[dimension] + 1e-10:
                        within_bounds_bool = False
                        break
                if within_bounds_bool:
                    valid_neighbours.append(corner)
            
            state_indices = []
            for neighbour in valid_neighbours:
                state_indices.append(neighbour[0] * self._num_bins + neighbour[1])
            

            weights = []
            
            for corner in valid_neighbours:
                w = 1.0
                for i in range(self.num_dims):
                    if corner[i] == lower_corner[i]:
                        w *= (1 - alpha[i])
                    else:
                        w *= alpha[i]
                weights.append(w)
            ...
            return list(zip(state_indices, normalized_weights))
    ...
    def compute_policy(self, max_iterations: int = 100, min_iter: int = 5, eval_sample_size: int = 15) -> None:
        ...
        elif self._policy_type == "deterministic_pi":  # deterministic_pi
    ...
    def _value_iteration(self, max_iterations: int, min_iter: int, eval_sample_size: int) -> None:
        ...
        for i in range(max_iterations):
            ...
            Q = np.sum(T * (R + self.gamma * new_values[None, None, :]), axis=2) 

    
            value_function = np.max(Q, axis=1)  

    
            value_diff = float(np.max(np.abs(new_values - value_function)))

    
            max_q = np.max(Q, axis=1, keepdims=True)          
            mask = np.isclose(Q, max_q)                       
            number_of_times = mask.sum(axis=1, keepdims=True)          
            policy = mask / np.maximum(number_of_times, 1)             
        ...
        for i in range(max_iterations):
            ...
            new_values = np.array(value_function, copy = True)
            for s in range(self.solver.num_states):
                ...
                for a in range(self.solver.num_actions):
                    ...
                    for s_next in range(self.solver.num_states):
                        action_value_sa += (T[s, a, s_next] * R[s, a, s_next]) + (T[s, a, s_next] * self.gamma * new_values[s_next])
                    action_values.append(action_value_sa) 
            ...
            for s in range(self.solver.num_states):
                ...
                for a in range(self.solver.num_actions):
                    q_sa_values = 0.0
                    for s_next in range(self.solver.num_states):
                        q_sa_values += (T[s, a, s_next] * R[s, a, s_next]) + (T[s, a, s_next] * self.gamma * value_function[s_next])
                    q_values.append(q_sa_values)
    ...
    def _deterministic_policy_iteration(
        ...
        self.solver._reward_function * policy[:, :, np.newaxis], axis=1
            ...
            for _ in range(horizon):
                V_new = np.sum(policy_T * (policy_R + self.gamma * value_function[None, :]), axis=1)
            ...
            T = self.solver._transition_function
            R = self.solver._reward_function
            Q = np.sum(T * (R + self.gamma * value_function[None, None, :]), axis=2)  
            ...
            policy_stable = np.array_equal(np.argmax(policy, axis=1), best_actions)
            policy = new_policy
        ...
        reward, steps = self.solve(max_steps=200, visualize=False)
        ...
        for k in range(50):
            ...
            for _ in range(horizon):
                v_temp = np.array(v_i, copy = True)
                delta_values = []
                for s in range(unwrapped_env.num_states):
                    a_policy = int(np.argmax(p_i[s]))
                    v_value = 0.0
                    for s_next in range(unwrapped_env.num_states):
                        v_value += (T[s, a_policy, s_next] * R[s, a_policy, s_next]) + (T[s, a_policy, s_next] * self.gamma * v_temp[s_next])
                    v_i[s] = v_value
                    delta_values.append(abs(v_i[s] - v_temp[s]))

                delta = np.max(delta_values) 
                if delta < self.theta:
                    break    
            
            p_old = p_i.copy()                    # snapshot the current policy
            ...
            for s in range(unwrapped_env.num_states):
                ...
                for a in unwrapped_env.actions:
                    q_sa_values = 0.0
                    for s_next in range(unwrapped_env.num_states):
                        q_sa_values += (T[s, a, s_next] * R[s, a, s_next]) + (T[s, a, s_next] * self.gamma * v_i[s_next])
                    q_values.append(q_sa_values)
                q_values = np.array(q_values)
        ...
        for _ in range(sample_size):
            ...
            while not done and num_steps < max_steps:
                ...
                if self._mode == "nn":
                    ...
                    num_actions = self.solver.num_actions
                    blended_policy = np.zeros(num_actions, dtype=float)
                    for s_idx, w in discrete_states:
                        blended_policy += w * self.solver.get_policy(int(s_idx))

                    total = blended_policy.sum()
                    if not np.isfinite(total) or total <= 0.0:
                        blended_policy[:] = 1.0 / num_actions
                    else:
                        blended_policy /= total

                    action = int(np.random.choice(num_actions, p=blended_policy))
