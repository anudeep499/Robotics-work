...
class ParticleFilter:
    ...
    def predict(self, action: np.ndarray):
        ...
        for i in range(self.n_particles):
            ...
            noisy_w   = angular_vel + np.random.normal(0, self.alpha2)
            ...
            if self.env._check_collision(np.array([x_new, y_new])):
    ...
    def update(self, measurements: np.ndarray):
        ...
        for i in range(self.n_particles):
            ...
            expected_measurements = self._get_expected_measurements(particle_state)
            
            likelihood = 1.00
            for beams in range(len(measurements)):
                error = measurements[beams] - expected_measurements[beams]
                probability = (
                1.0 / (np.sqrt(2*np.pi) * self.sigma_range)) * np.exp(-(error**2) / (2 * self.sigma_range**2))
    ...
    def resample(self):
        ...
        selected_particles = np.random.choice(self.n_particles, size=self.n_particles, p=self.weights)
        refined_particles = self.particles[selected_particles].copy() 



        noise = np.random.normal(loc=0.0, scale=self.resampling_noise, size=refined_particles.shape)
        refined_particles += noise


        refined_particles[:, 2] = (refined_particles[:, 2] + np.pi) % (2 * np.pi) - np.pi

        self.particles = refined_particles
        self.weights = np.ones(self.n_particles) / self.n_particles
    ...
    def estimate_state(self) -> np.ndarray:
        ...
        weighted_average_x = np.sum(x * self.weights)
        weighted_average_y = np.sum(y * self.weights)
        weighted_average_theta = np.arctan2(np.sum(np.sin(angles) * self.weights), np.sum(np.cos(angles) * self.weights))

        return np.array([weighted_average_x, weighted_average_y, weighted_average_theta ])
